...
// Check this line below - I added "User" to the list
  ...
  return (
    <div className="flex min-h-screen bg-black text-white selection:bg-[#C5A059]/30">
      ...
      <main className="flex-1 p-8 md:p-12 relative overflow-hidden">
        {/* Half-Background Glow for that Executive Look */}
        ...
        <div className="max-w-5xl mx-auto backdrop-blur-md bg-white/5 p-8 md:p-12 rounded-[50px] border border-white/10 shadow-2xl">
          <header className="mb-12 flex flex-col md:row items-center gap-8 text-center md:text-left">
            
            {/* THE FIX: Clickable Profile Picture */}
            <label className="relative group cursor-pointer">
               <div className="w-32 h-32 md:w-40 md:h-40 bg-black border-2 border-[#C5A059]/30 rounded-full flex items-center justify-center text-[#C5A059] group-hover:border-[#C5A059] transition-all overflow-hidden relative">
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#C5A059]">Update Photo</span>
                  </div>
            ...
            <div>
              ...
              <p className="text-gray-500 text-[10px] uppercase tracking-widest mt-3 font-bold border-l border-[#C5A059] pl-3">Public Label Identity & DSP Metadata</p>
            </div>
          </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="space-y-6 flex flex-col">
               ...
               <textarea 
                  className="w-full flex-1 bg-black border border-white/10 p-6 rounded-[35px] outline-none focus:border-[#C5A059] transition-all text-sm h-48 md:h-full placeholder:text-gray-800 text-white"
                  placeholder="Tell the world your story..."
               />
               
               <button 
                  onClick={() => { setLoading(true); setTimeout(() => setLoading(false), 2000); }}
                  className="w-full bg-[#C5A059] text-black font-black py-6 rounded-[30px] uppercase text-[10px] tracking-[0.4em] hover:bg-white transition-all shadow-xl shadow-[#C5A059]/10"
               >
  ...
  return (
    <div>
      ...
      <div className="relative">
         ...
         <input 
            {...props} 
            className="w-full bg-black border border-white/10 p-5 pl-16 rounded-[25px] outline-none focus:border-[#C5A059] transition-all text-sm text-white placeholder:text-gray-800" 
         />
      </div>
    </div>
  )
