...
export default function Sidebar() {
  ...
  return (
    <aside className="w-72 h-screen bg-black border-r border-white/5 p-8 flex flex-col sticky top-0 z-50">
      ...
      <nav className="flex-1 space-y-2">
        {menu.map((item) => (
          <button 
            ...
            className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-gray-400 hover:bg-white/5 hover:text-[#C5A059] transition-all"
          >
            <item.icon size={20} />
      ...
      {/* THIS BUTTON NOW TRIGGERS THE UPLOAD */}
      <button 
        onClick={() => alert('Opening Upload Vault... (Linking to Storage now)')}
        className="mt-auto w-full bg-[#C5A059] text-black font-black py-5 rounded-3xl flex items-center justify-center gap-2 hover:scale-105 transition-transform shadow-xl shadow-[#C5A059]/10"
      >
        <PlusCircle size={20} />
